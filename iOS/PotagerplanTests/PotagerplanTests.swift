import XCTest
@testable import Potagerplan

final class PotagerplanTests: XCTestCase {
    ...
    func test_addEntry_increasesCount() {
        ...
        let entry = Entry(bedName: "Test", date: Date(), note: "note")
    ...
    func test_addEntry_blockedAtFreeLimit() {
        ...
        while store.entries.count < Store.freeLimit {
            _ = store.add(Entry(bedName: "Filler", date: Date(), note: ""), isPro: false)
        }
        let result = store.add(Entry(bedName: "Overflow", date: Date(), note: ""), isPro: false)
    ...
    func test_addEntry_proBypassesLimit() {
        ...
        let result = store.add(Entry(bedName: "ProEntry", date: Date(), note: ""), isPro: true)
    ...
    func test_deleteEntry_removesFromList() {
        let store = Store()
        let entry = Entry(bedName: "ToDelete", date: Date(), note: "")
        _ = store.add(entry, isPro: false)
        store.delete(entry)
    ...
    func test_updateEntry_changesFields() {
        let store = Store()
        var entry = Entry(bedName: "Original", date: Date(), note: "")
        _ = store.add(entry, isPro: false)
        entry.bedName = "Updated"
        store.update(entry)
        XCTAssertEqual(store.entries.first(where: { $0.id == entry.id })?.bedName, "Updated")
    }
