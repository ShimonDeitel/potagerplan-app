...
struct EntryFormView: View {
    ...
    init(entry: Entry?, onSave: @escaping (Entry) -> Void) {
        _fieldValue = State(initialValue: entry?.bedName ?? "")
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                Form {
                    Section("Bed name") {
                        TextField("Bed name", text: $fieldValue)
                            .accessibilityIdentifier("entryFieldInput")
                    }
                    Section("Planned") {
                        DatePicker("Date", selection: $date, displayedComponents: .date)
                    }
                    Section("Pairing Notes") {
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        let entry = Entry(
                            id: entryID ?? UUID(),
                            bedName: fieldValue.isEmpty ? "Untitled" : fieldValue,
                            date: date,
                            note: note
                        )
                        onSave(entry)
